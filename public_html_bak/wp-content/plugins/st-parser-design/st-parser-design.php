...
function st_parser_design(){global $wpdb, $cur_user_id;  /*// var_dump(st_parser_add_task($url, $region, $page)); //*/
    $citys = ['msk' => 'Москва',
              'spb' => 'Санкт-Петербург', 
              'kzn' => 'Казань', 
              'krr' => 'Краснодар', 
    ...
    $sqlR = $wpdb->prepare("SELECT * FROM parser WHERE id_user = %d AND `delete` = 0  ORDER BY time DESC", $cur_user_id);
