    ...
    public function html(){
        ...
        <div class="bp-messages-wrap bp-better-messages-list">
            ...
            <?php if(in_array('messages', $tabs)){
                ...
                $new_thread_button = false;
                if( BP_Better_Messages()->settings['disableNewThread'] === '0' || current_user_can('manage_options') ){
                    $new_thread_button = true;
                }
                ?>
                <div class="messages<?php if( ! $new_thread_button ) echo ' no-new-thread-button'; ?>">
                    ...
                    <?php } ?>
                    <?php if( $new_thread_button ) { ?>
                        <div class="chat-header">
                            <a href="<?php echo add_query_arg( 'new-message', '', BP_Better_Messages()->functions->get_link() ); ?>&scrollToContainer" class="new-message ajax" title="<?php _e( 'New Thread', 'bp-better-messages' ); ?>"><i class="fas fa-plus" aria-hidden="true"></i></a>
                        </div>
                    <?php } ?>
                    </div>
            <?php } ?>
