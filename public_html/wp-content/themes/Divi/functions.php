...
function redirect_login_page(){
    //if (function_exists('st_login_form_param')) {
    //    extract(st_login_form_param());
    //} else {
        $Loginurl = (true) ?home_url() : get_page_link(333);
    //}
    //echo $Loginurl;
    $page_viewed = basename($_SERVER['REQUEST_URI']);  
    $crt = $_SERVER['SCRIPT_NAME'];
    if (is_user_logged_in()) {
    } else {
    if ($page_viewed == 'wp-login' or $page_viewed  = 'wp-login.php' OR $crt = '/wp-login.php' ) {
        $get = http_build_query($_GET);
        wp_redirect( $Loginurl.'?'.$get );  
    }
    }
}    
...
/*//Личный кабинет парсера -> Дополнительные поля клиента //*/
function add_extra_lk($user){$balance = get_the_author_meta('balance', $user->ID); $balance = (empty($balance)) ? 0 : $balance;
global $tarif; 
$tarif = array('На 1 день (24 часа) - 250 рублей.','7 дней (168 часов) - 1250 рублей.','На 30 дней (720 часов) - 3500 рублей.');

global $tarif2; 
$tarif2 = array('На 1 день (24 часа) - 250 рублей.','7 дней (168 часов) - 1250 рублей.','На 30 дней (720 часов) - 3500 рублей.');
?>
...
<table class="form-table">
 <tr>
  ...
  <td><input type="number" min="0" id="myInputNumber" name="balance" value="<?=esc_attr($balance);?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">Телефон</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(get_the_author_meta('phone', $user->ID));?>" class="regular-text" /></td>
 </tr>
 <tr>
  <th><label for="phone">Тариф парсера WB</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
  <tr>
  <th><label for="phone">Тариф парсера Yandex</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif2($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
   <tr>
  <th><label for="phone">Тариф парсера Lamoda</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif3($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">Тариф парсера Ozon</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif4($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">Тариф парсера Sber</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif5($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">Тариф парсера KazanExpress</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif6($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">Тариф парсера Aliexpress</label></th>
  <td><input type="text" name="phone" value="<?=esc_attr(extra_tarif7($user->ID)['name']);?>" class="regular-text" disabled/></td>
 </tr>
</table>
<?php
}

add_action('show_user_profile','add_extra_lk'); add_action('edit_user_profile','add_extra_lk');

function save_extra_lk($user_id){if(empty($user_id) && !is_numeric($user_id)){return false;}
    global $_POST;
    //if(empty($_POST['balance'])){$_POST['balance'] = '0';}
    if(empty($_POST['tarif'])){$_POST['tarif'] = '1@'.time();}
    if(empty($_POST['tarif2'])){$_POST['tarif2'] = '1@'.time();}
    if(empty($_POST['tarif3'])){$_POST['tarif3'] = '1@'.time();}
    if(empty($_POST['tarif4'])){$_POST['tarif4'] = '1@'.time();}
    if(empty($_POST['tarif5'])){$_POST['tarif5'] = '1@'.time();}
     if(empty($_POST['tarif6'])){$_POST['tarif6'] = '1@'.time();}
      if(empty($_POST['tarif7'])){$_POST['tarif7'] = '1@'.time();}
    if((int) $_POST['balance'] < 0){unset($_POST['balance']);}

    foreach($_POST as $kp => $p){
        if('balance'==$kp || 'phone'==$kp || 'tarif'==$kp || 'tarif2'==$kp || 'tarif3'==$kp || 'tarif4'==$kp || 'tarif5'==$kp || 'tarif6'==$kp || 'tarif7'==$kp){ 
        $old = get_the_author_meta($kp, $user_id);
        if('tarif'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif', sanitize_text_field($p));
            }else{continue;}
             
        }elseif($p && $p != $old){
            update_user_meta($user_id, $kp, sanitize_text_field($p));
        }

if('tarif2'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif2($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif2', sanitize_text_field($p));
            }else{continue;}
        ...
        if('tarif3'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif3($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif3', sanitize_text_field($p));
            }else{continue;}
        ...
        if('tarif4'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif4($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif4', sanitize_text_field($p));
            }else{continue;}
        ...
        if('tarif5'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif5($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif5', sanitize_text_field($p));
            }else{continue;}
        ...
        if('tarif6'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif6($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif6', sanitize_text_field($p));
            }else{continue;}
        ...
        if('tarif7'==$kp && ('0' != $p[0])){
            $b = (int) get_the_author_meta('balance', $user_id);
            //$t = extra_tarif($user_id,true)[$old[0]]; $t['cash'] = (int) $t['cash'];  
            $tp = extra_tarif7($user_id,true)[$p[0]]; $tp['cash'] = (int) $tp['cash'];  
             
            if($p[0] != $old[0] && $b > $tp['cash']){
                update_user_meta($user_id, 'balance', sanitize_text_field($b-$tp['cash']));
                update_user_meta($user_id, 'tarif7', sanitize_text_field($p));
            }else{continue;}
        ...
        if(empty($p) && $old){
            delete_user_meta($user_id, $kp, $old);
    }}}return true;                                                        
}

add_action('personal_options_update','save_extra_lk'); add_action('edit_user_profile_update','save_extra_lk');

function extra_tarif($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
                3 => array('cash' => '1250', 'worktime' => '168', 'name' => 'На 7 дней (168 часов) - 1250 рублей. '),
                4 => array('cash' => '3500', 'worktime' => '720', 'name' => 'На 30 дней (720 часов) - 3500 рублей.')
    );
    if($view){return $t;}
    
    $tarif = explode('@', get_the_author_meta('tarif', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
    }}}
    return false;
}

function extra_tarif2($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif2', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
function extra_tarif3($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif3', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
function extra_tarif4($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif4', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
function extra_tarif5($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif5', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
function extra_tarif6($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif6', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
function extra_tarif7($user_id, $view = false){
    if(empty($user_id) && !is_numeric($user_id)){return false;}
    $t = array( 0 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Лимит исчерпан.'),
                1 => array('cash' => ''    , 'worktime' => ''   , 'name' => 'Один бесплатный парсинг.'),
                2 => array('cash' => '250' , 'worktime' => '24' , 'name' => 'На 1 день (24 часа) - 250 рублей.'),
    ...
    $tarif = explode('@', get_the_author_meta('tarif7', $user_id), 2, 1);
    if(is_array($tarif)){
        if(is_numeric($tarif['id'] = $tarif[0])){unset($tarif[0]);
            if($tarif['time'] = (ctype_digit($tarif[1]) && strtotime(date('Y-m-d H:i:s',$tarif[1])) === (int)$tarif[1]) ? $tarif[1] : false){unset($tarif[1]);
                if($tarif = array_merge($tarif, $t[$tarif['id']])){return $tarif;} 
...
add_action('user_register', function($user_id){
    ...
    update_user_meta($user_id, 'tarif', '1@'.time());
    update_user_meta($user_id, 'tarif2', '1@'.time());
    update_user_meta($user_id, 'tarif3', '1@'.time());
    update_user_meta($user_id, 'tarif4', '1@'.time());
    update_user_meta($user_id, 'tarif5', '1@'.time());
    update_user_meta($user_id, 'tarif6', '1@'.time());
    update_user_meta($user_id, 'tarif7', '1@'.time());
});