...
function st_parser_add_task2($parse_url, $region, $page){global $wpdb, $cur_user_id;
    ...
    $Zapros='http://188.120.227.124:8000/put/'.$cur_user_id.'?q='.$url2.'&p='.$page;
    ...
    $WPSaveData['id_parser'] = 2;
    $wpdb->insert( 'parser', $WPSaveData);
    $lID = $wpdb->insert_id;
    return $lID;
}
function st_parser_checkstatus2(int $id){global $wpdb, $cur_user_id; $ret = false;
    $sqlR = $wpdb->prepare("SELECT *FROM parser WHERE id_user = '%d' AND id = '%d'", [$cur_user_id, $id]);
    $data = $wpdb->get_row($sqlR, 'ARRAY_A');
    if($data){$Cdata = st_parser_curl('http://188.120.227.124:8000/check/'.$cur_user_id.'?task_id='.$data['id_task']);    
...
function st_parser_design2(){global $wpdb, $cur_user_id;  $cur_user_id = intval(get_current_user_id()); /*// var_dump(st_parser_add_task($url, $region, $page)); //*/
    $citys = ['msk' => 'Москва',
              'spb' => 'Санкт-Петербург', 
              'kzn' => 'Казань', 
              'krr' => 'Краснодар', 
              'nsk' => 'Новосибирск', 
              'hbr' => 'Хабаровск', 
              'ekb' => 'Екатеринбург']; $f = (string) @get_the_author_meta('tarif2', $cur_user_id)[0]; $b = (string) @get_the_author_meta('balance', $cur_user_id);
    if(is_user_logged_in()){
        ...
        if(!empty($_POST['parser_on'])){update_user_meta($cur_user_id, 'tarif2', '0'); if('1' == $f){echo '<script type="text/javascript">location.replace(location);</script>';}}
        if(!empty($_POST['parser_s'])){$page = (!empty($_POST['parser_s']) && is_numeric($_POST['parser_page'])) ? intval($_POST['parser_page']) : 1;
            ...
            st_parser_add_task2($url, $region, $page);                                                                      
        }else if($_GET['status'] = (int)$_GET['status']){
            $rt = st_parser_checkstatus2($_GET['status']);
        }else if(isset($_GET['del']) && $_GET['del'] = (int)$_GET['del']){        
            $wpdb->update('parser', ['delete'=>1], ['id' => $_GET['del'], 'id_user'=>$cur_user_id]);    
        }
    $sqlR = $wpdb->prepare("SELECT * FROM parser WHERE id_user = %d AND `delete` = 0 AND id_parser = 2 ORDER BY time DESC", $cur_user_id);
...
<?foreach(extra_tarif2($cur_user_id,true) as $kt => $t){//*/
...
<?php if($val['status'] == 2){ $ZaprosGet = "this.href='http://188.120.227.124:8000/get_result/$cur_user_id?task_id=".$val['id_task']."'";?>
                    <td>
                        <!--a target="_blank" href="/" onclick="<?=$ZaprosGet?>" class="stc-report-button st-but-compl">Выгрузить</a-->
                         <a target="_blank" href="<?=$plugins_url = plugins_url('st-parser-design/st-parser-get.php' );?>?do_task=<?=$val['id_task'];?>&uid=<?=$cur_user_id;?>&id_parser=2" class="stc-report-button st-but-compl">Выгрузить</a>
                    </td>
<?php }else if($val['status'] == 1){?>
                    <td>
                        <a href="/parser-yandex/?status=<?=$val['id'];?>" class="stc-report-button st-but-compl">Обновить статус</a>
                    </td>
<?php } ?>
                    <td>
                        <a href="/parser-yandex/?del=<?php echo $val['id']; ?>" class=""><i class="far fa-trash-alt"></i></a>
                    </td>
                </tr>
<?php $sort++;} ?>
            </tbody>
...
add_shortcode('parserdesign2', 'st_parser_design2');
