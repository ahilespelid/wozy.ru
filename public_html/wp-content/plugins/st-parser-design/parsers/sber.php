...
function st_parser_add_task5($parse_url, $region, $page){
    global $wpdb, $cur_user_id;
    ...
    $Cdata = st_parser_curl_post5($Zapros);
    

    if (200 == $Cdata['status']) { 
        $WPSaveData['status'] = 1;
        $tmp1 = json_decode($Cdata['data'], true);
        $RemoteID = (string)$tmp1['req_id'];
    ...
    $WPSaveData['id_parser'] = 5;
    $wpdb->insert( 'parser', $WPSaveData);
    $lID = $wpdb->insert_id;
    return $lID;
}
function st_parser_checkstatus5(int $id){
    global $wpdb, $cur_user_id; $ret = false;
    $sqlR = $wpdb->prepare("SELECT *FROM parser WHERE id_user = '%d' AND id = '%d'", [$cur_user_id, $id]);
    $data = $wpdb->get_row($sqlR, 'ARRAY_A');
    if($data){
        $Cdata = st_parser_curl('http://149.154.64.48:8500/get_req_id_status/'.$data['id_task']);    
        if(200 == $Cdata['status']){
            $ret2 = json_decode ($Cdata['data'], true);
            $ret = $ret2[$data['id_task']];
        }
        if(isset($ret['is_complete']) or isset($ret['in_process'])){           
            $updWhere['id_user'] = $cur_user_id; $updWhere['id'] = $id; 
            $upd1['status'] = ($ret['is_complete']) ? 2 : (($ret['in_process']) ? 1 : '');
            $rTemp1 = $wpdb->update( 'parser', $upd1, $updWhere);           
    }}return $ret;
}
function st_parser_design5(){global $wpdb, $cur_user_id;  $cur_user_id = intval(get_current_user_id()); /*// var_dump(st_parser_add_task($url, $region, $page)); //*/
    $citys = ['msk' => 'Москва',
              'spb' => 'Санкт-Петербург', 
              'kzn' => 'Казань', 
              'krr' => 'Краснодар', 
              'nsk' => 'Новосибирск', 
              'hbr' => 'Хабаровск', 
              'ekb' => 'Екатеринбург']; $f = (string) @get_the_author_meta('tarif5', $cur_user_id)[0]; $b = (string) @get_the_author_meta('balance', $cur_user_id);
    if(is_user_logged_in()){
        ...
        if(!empty($_POST['parser_on'])){
            update_user_meta($cur_user_id, 'tarif5', '0'); if('1' == $f){echo '<script type="text/javascript">location.replace(location);</script>';}}
        if(!empty($_POST['parser_s'])){
            $page = (!empty($_POST['parser_s']) && is_numeric($_POST['parser_page'])) ? intval($_POST['parser_page']) : 1;
            $region = (!empty($_POST['parser_region'])) ? $_POST['parser_region'] : ''; $url = $_POST['parser_s'];
            st_parser_add_task5($url, $region, $page);                                                                      
        }else if($_GET['status'] = (int)$_GET['status']){
            $rt = st_parser_checkstatus5($_GET['status']);
        }else if(isset($_GET['del']) && $_GET['del'] = (int)$_GET['del']){        
            $wpdb->update('parser', ['delete'=>1], ['id' => $_GET['del'], 'id_user'=>$cur_user_id]);    
        }
    $sqlR = $wpdb->prepare("SELECT * FROM parser WHERE id_user = %d AND `delete` = 0 AND id_parser = 5 ORDER BY time DESC", $cur_user_id);
...
<?foreach(extra_tarif4($cur_user_id,true) as $kt => $t){//*/
...
<?php if($val['status'] == 2){ $ZaprosGet = "this.href='http://188.120.227.124:8000/get_result/$cur_user_id?task_id=".$val['id_task']."'";?>
                    <td>
                        <!--a target="_blank" href="/" onclick="<?=$ZaprosGet?>" class="stc-report-button st-but-compl">Выгрузить</a-->
                         <a target="_blank" href="<?=$plugins_url = plugins_url('st-parser-design/st-parser-get2.php' );?>?do_task=<?=$val['id_task'];?>&uid=<?=$cur_user_id;?>&id_parser=5" class="stc-report-button st-but-compl">Выгрузить</a>
                    </td>
<?php }else if($val['status'] == 1){?>
                    <td>
                        <a href="/parser-sber/?status=<?=$val['id'];?>" class="stc-report-button st-but-compl">Обновить статус</a>
                    </td>
<?php } ?>
                    <td>
                        <a href="/parser-sber/?del=<?php echo $val['id']; ?>" class=""><i class="far fa-trash-alt"></i></a>
                    </td>
                </tr>
<?php $sort++;} ?>
            </tbody>
...
add_shortcode('parserdesign5', 'st_parser_design5');



function st_parser_curl_post5($data){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://149.154.64.48:8500/post_user_request/');
