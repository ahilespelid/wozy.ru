...
(function ($) {
    ...
    function BPBMUpdateUnreadCount( unread ){
        $(document).trigger("bp-better-messages-update-unread", unread);
    ...
    $(document).ready(function () {
         ...
         * Mute thread
         */
        bpMessagesWrap.on('click', '.chat-header .bpbm-leave-thread', function (event) {
            event.preventDefault();
            var container = $(this).closest('.bp-messages-wrap');
    ...
    function playSound( message_id ) {
        if( BP_Messages['enableSound'] !== '1' ) return false;
