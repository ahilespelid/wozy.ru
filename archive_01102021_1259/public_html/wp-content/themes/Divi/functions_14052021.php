...
add_filter('pre_site_transient_update_themes','__return_null'); add_filter('auto_update_theme','__return_false'); add_action('admin_menu','hide_admin_notices');
if(!isset($content_width)){$content_width=1080;}
if(!function_exists(pa)){function pa($a){echo '<pre>'; print_r($a); echo '</pre>';}}
if(!empty($user)){pa($user);}
pa($user);
...
function et_setup_theme(){
    ...
    $themename='Divi'; $shortname='divi'; $et_store_options_in_one_row=true; $default_colorscheme="Default"; $template_directory=get_template_directory();
    require_once($template_directory.'/core/init.php'); et_core_setup(get_template_directory_uri());    
    if('3.0.61'===ET_CORE_VERSION){require_once $template_directory.'/core/functions.php'; require_once $template_directory.'/core/components/init.php'; et_core_patch_core_3061();}
    ...
    if(wp_doing_cron()){et_register_updates_component();}
}add_action('after_setup_theme','et_setup_theme');
function et_divi_load_unminified_scripts($load){
    if(et_core_is_safe_mode_active()){return true;}
    if('false'===et_get_option('divi_minify_combine_scripts')){return true;}
    return $load;
}
function et_divi_load_unminified_styles($load){
    if(et_core_is_safe_mode_active()){return true;}
    if('false'===et_get_option('divi_minify_combine_styles')){return true;}
    return $load;
}
function et_theme_epanel_reminder(){global $shortname, $themename;
    $documentation_url='http://www.elegantthemes.com/gallery/divi/readme.html'; $documentation_option_name=$shortname.'_2_4_documentation_message';
    if(false===et_get_option($shortname.'_logo') && false===et_get_option($documentation_option_name)){
        ...
        printf('<div class="notice is-dismissible" style="background-color: #6C2EB9; color: #fff; border-left: none;"><p>%1$s</p></div>',$message);
...
add_filter('show_admin_bar', '__return_false');
/*//Р РµРґРёСЂРµРєС‚ РїСЂРё Р°РІС‚РѕСЂРёР·Р°С†РёРё//*/
function redirect_login_page(){
    if (function_exists('st_login_form_param')) {
        extract(st_login_form_param());
    } else {
        $Loginurl = get_page_link(306);
    }
...
/*//Р›РёС‡РЅС‹Р№ РєР°Р±РёРЅРµС‚ РїР°СЂСЃРµСЂР° -> Р”РѕРїРѕР»РЅРёС‚РµР»СЊРЅС‹Р№ РїРѕР»СЏ РєР»РёРµРЅС‚Р° //*/
function add_extra_lk($user){?>
...
<table class="form-table">
 <tr>
  <th><label for="balance">Р‘Р°Р»Р°РЅСЃ</label></th>
  <td><input type="number" min="0" id="myInputNumber" name="balance" value="<?=esc_attr(get_the_author_meta('balance', $user->ID));?>" class="regular-text" disabled/></td>
 </tr>
 <tr>
  <th><label for="phone">РўРµР»РµС„РѕРЅ</label></th>
...
function save_extra_lk($user_id){
    update_user_meta($user_id,'balance',sanitize_text_field($_POST['balance']));
    update_user_meta($user_id,'phone',sanitize_text_field($_POST['phone']));
}add_action('personal_options_update','save_extra_lk'); add_action('edit_user_profile_update','save_extra_lk');

/*// side | advanced | normal - отображать ниже редактора  //*/
/*// hight — высокий, default — по умолчанию, low — низкий //*/
function my_meta_box(){
    add_meta_box('my_meta_box','Личный кабинет','show_my_metabox','post','normal','high');
}add_action('add_meta_boxes', 'my_meta_box');
$meta_fields = array(
    array(  
        'label' => 'Текстовое поле',  
        'desc'  => 'Описание для поля.',  
        'id'    => 'mytextinput',           /*// даем идентификатор html //*/
...
function show_my_metabox() {/*// вызов метаполей                                                     //*/   
global $meta_fields;        /*// обозначим наш массив с полями глобальным                            //*/
global $post;               /*// глобальный $post для получения id создаваемого/редактируемого поста //*/
                            /*// выводим скрытый input, для верификации. Безопасность прежде всего!  //*/
echo '<input type="hidden" name="custom_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';  
echo '<table class="form-table">'; /*// начинаем выводить таблицу с полями через цикл //*/
    foreach($meta_fields as $field){
        $meta = get_post_meta($post->ID, $field['id'], true); /*// получаем значение если оно есть для этого поля //*/
        echo '<tr>
...
              <span class="description">'.$field['desc'].'</span>'; break;
    case 'textarea':  
        echo '<textarea name="'.$field['id'].'" id="'.$field['id'].'" cols="60" rows="4">'.$meta.'</textarea><br />
              <span class="description">'.$field['desc'].'</span>'; break;
    case 'checkbox':  
        echo '<input type="checkbox" name="'.$field['id'].'" id="'.$field['id'].'" ',$meta ? ' checked="checked"' : '','/>
              <label for="'.$field['id'].'">'.$field['desc'].'</label>'; break; 
    case 'select':  
        echo '<select name="'.$field['id'].'" id="'.$field['id'].'">';  
        foreach($field['options'] as $option){  
        echo '<option', $meta == $option['value'] ? ' selected="selected"' : '', ' value="'.$option['value'].'">'.$option['label'].'</option>';}  
        echo '</select><br /><span class="description">'.$field['desc'].'</span>'; break;
                }
        echo '</td></tr>';  
    }  
    echo '</table>';
...
    if(defined('DOING_AUTOSAVE') && DOING_AUTOSAVE){return $post_id;} /*// авто-сохранение //*/
    /*// права доступа //*/ 
...
    }elseif(!current_user_can('edit_post', $post_id)){return $post_id;}  
    /*// обновляем данные //*/
    foreach($meta_fields as $field){  
        $new = $_POST[$field['id']]; $old = get_post_meta($post_id, $field['id'], true);
        if($new && $new != $old) {      /*// если данные новые обновляем //*/
            update_post_meta($post_id, $field['id'], $new);
        } elseif('' == $new && $old){   /*// если данных нету, удаляем мету //*/
