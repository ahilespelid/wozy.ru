...
if ($_GET['id_parser'] == 1) {
	...
	} elseif ($_GET['id_parser'] == 6) {
		$get_pars_file = 'http://92.63.192.39:8082/get_j/'.$_GET['uid'].'?task_id='.$_GET['do_task'];
		$spreadsheet = PhpOffice\PhpSpreadsheet\IOFactory::load('sh6.xlsx');
	} elseif ($_GET['id_parser'] == 7) {
		$get_pars_file = 'http://188.120.255.121:8000/get_j/'.$_GET['uid'].'?task_id='.$_GET['do_task'];
		$spreadsheet = PhpOffice\PhpSpreadsheet\IOFactory::load('sh7.xlsx');
	}
	...
	foreach ($content as $key => $value) {
		...
		$sheet->getColumnDimension("$string")->setWidth(30);
			...
			$sheet->setCellValueByColumnAndRow($i2, $i, (string)$value2);
